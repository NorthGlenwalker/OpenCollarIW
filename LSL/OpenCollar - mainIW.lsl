...
integer g_iSpyMenu=FALSE;//remove trace if spy menu installed
...
AppsMenu(key kID, integer iAuth)
{
    ...
    else if(g_iSpyMenu == TRUE && g_iTraceOn == TRUE)
    {
        g_iTraceOn = FALSE; //force to turned off since you can no longer access it
...
default
{
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        if (iNum == COMMAND_NOAUTH)
            return;
        else if (sStr == "504|504|504" || sStr == "504|504|504|504") // it was spamming this at one time??? so dump it just in case
            return;
        // /SA
