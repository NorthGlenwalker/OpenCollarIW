...
{
    string sPrompt = "<pre>access or <pre>owners - this menu\neach button will give the chat command\nSelect the level and then person for access of your "+CTYPE+".";
...
{ // here iNum: auth value, sStr: user command, kID: avatar id  
    ...
    llOwnerSay("<pre>owner - opens the add owner select dialog box
<pre>tempowner - opens the add temp owner select dialog box
<pre>secowner - opens the add secondary owner select dialog box
<pre>blacklist - opens the add a person to the colalr blacklist dialog box");
...
//        else if ((key)sTmpName)//NG - else if ((key)sTmpName) don't work in IW
    ...
    llOwnerSay("<pre>remowner or <pre>remowners - opens the add owner select dialog box
<pre>remtempowner - opens the remove temp owner select dialog box
<pre>remsecowner - opens the remove secondary owner select dialog box
<pre>remblacklist - opens the remove a person to the collar blacklist dialog box");
        if (sCommand=="remowners")
            sCommand="remowner";
    ...
    {
        llOwnerSay("<pre>setgroup - set group access to your current group");
        if (iNum==COMMAND_OWNER)
        {
    ...
    {
        llOwnerSay("<pre>setgroupname [group name] - sets group access to the group [group name]");
        if (iNum==COMMAND_OWNER)
        {
    ...
    {
        llOwnerSay("<pre>unsetgroup - removes group access");
        if (iNum==COMMAND_OWNER)
        {
            g_kGroup = "";
            g_sGroupName = "";
    ...
    {
        llOwnerSay("<pre>setopenaccess - puts the collar into open mode");
        if (iNum==COMMAND_OWNER)
        {
            g_iOpenAccess = TRUE;
    ...
    {
        llOwnerSay("<pre>unsetopenaccess - removes open access mode form the collar");
        if (iNum==COMMAND_OWNER)
        {
            g_iOpenAccess = FALSE;
    ...
    {
        llOwnerSay("<pre>setlimitrange - limits the access range to 20M");
        if (iNum==COMMAND_OWNER)
        {
            g_iLimitRange = TRUE;
    ...
    {
        llOwnerSay("<pre>unsetlimitrange - removes the limits of access range");
        if (iNum==COMMAND_OWNER)
        {
            g_iLimitRange = FALSE;
    ...
    {
        llOwnerSay("<pre>runaway - starts the runaway procerss");
        list lButtons=[];
