...
{
    string sPrompt = "<pre>undress - this menu
<pre>smartstrip=on (off) - turns smart strip on or off
<pre>strip=all - trys to removes all clothing and worn items
...
{
    string sPrompt = "<pre>lockclothingmenu - this menu\nSelect an article of clothing to un/lock.";
    list lButtons;
...
{
    string sPrompt = "<pre>lockattachmentmenu - this menu\nSelect an attachment to un/lock.";
    list lButtons;
...
DetachMenu(key kID, string sStr, integer iAuth)
{
    ...
    //for each 1, add capitalized button
    string sPrompt = "<pre>attachment - this menu\nSelect an attachment to remove.";
