...
integer compareVersions(string v1, string v2)
{ //compares two symantic version strings, true if v1 >= v2
...
<pre>apps or <pre>addons - this menu
<pre>trace or <pre>trace off - turns trace off
<pre>trace on - turns trace on
<pre>garble on - turns on garble if fitted
<pre>garble off - turns off garble if fitted
Trace will not show in collar if Spy App installed
";
    ...
    if (g_iTraceOn)
    {
        ...
        sPrompt +="\n Trace function is currently OFF\nBy turning this ON all Owners will be notified when the collar wearer Teleports";
...
{
    string sPrompt="\nWearer only access\n";
    sPrompt +="<pre>about - this menu
<pre>help - gives the Quick Guide(help)
<pre>license - give the License
<pre>update - starts the collar updater
<pre>news on - turns the update news on
<pre>news off - turns the update news off
";
...
MainMenu(key kID, integer iAuth)
{
    ...
    sPrompt +="\nChat Commands <pre> = prefix letters to access this collar.
This menu -
<pre>menu - single collar access
?menu - all collars you have access to except yourself
*menu - all collars you have access to in range
<pre>lock - Lock the collar
<pre>unlock - Unlock the collar
<pre>togglelock - Toggle the lock/unlock state of the collar
<pre>version - shows the current collar version number.";
    if(!g_iLatestVersion)
        sPrompt+="\nUpdate available!";
