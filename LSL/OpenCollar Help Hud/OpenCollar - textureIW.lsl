...
//build textures notecard if not exist
string texture_notecard = "Blank,f966405d-40e3-4d09-8b8f-4154e824fec9
Brass,8bc6fe12-7065-4add-bac6-1821e95ddda1
Chrome,9129c6a7-e78a-4e6c-a951-635498c8ca0f
Fur Dark,a5652cd3-c432-40b8-ab56-324722a0b5a6
Fur Light,73a599e7-f4b4-4079-acb4-7a530967021f
Gem,e8579829-ef1b-42cd-8a6e-84e757bf6ac3
Gold,d03866e3-eddc-432d-8411-959849013d8a
Grunge,b65949ba-6ccf-4cd8-9bd1-7e590cfbfa2b
Lace,9e7f48cdb-2cab-4001-bc0f-90ab8617944e
Leather Dark,c5738c5c-3572-4260-9f0c-f0b4b2403967
Leather Light,d95f36dd-126e-4d47-9420-acaf48decaba
Rust,01e48724-1390-4be8-adde-7eba0549da75
Silver,14e58d74-2590-4f71-92a3-152043ce2a9e
Snake,5f9a679d-db33-4df6-87fc-a35587252baf
Steel,7440aafc-c167-4d29-9e56-0b3dc029e85d";

loadNotecardTextures()
{
    g_sTextureCard=g_sDefTextureCard+"_custom";
    if(llGetInventoryType(g_sTextureCard)!=INVENTORY_NOTECARD)
        g_sTextureCard=g_sDefTextureCard;
    if(llGetInventoryType(g_sTextureCard)!=INVENTORY_NOTECARD)
    {
...
{
    if (llGetSubString(sInvName, 0, 5) == "leash_")
        return TRUE;
    if (sInvName == "chain" || sInvName == "rope" || sInvName == "!totallytransparent")
        return TRUE;
    return FALSE;
}
...
string GetLongName(string ele, string sTex) // find the full texture name given element + shortname
{
    ...
    for (; l < llGetListLength(work); l++)
    {
        ...
        {
            if (!GetElementHasTexs(ele) || ~llSubStringIndex(test, ele + SEPARATOR))
                return test; //KW
        }
    }
...
integer GetElementHasTexs(string ele) // check if textures exist with labels for the specified element
{
    ...
    for (; l < max; l++)
    {
         if (~llSubStringIndex(llList2String(g_lNotecardTextures,l), ele))
            return TRUE;
    }
    return FALSE;
}
...
list BuildTexButtons()
{
    ...
    list out = [];
    if (~llListFindList(g_lTextureDefaults, [s_CurrentElement]))
        out = ["Default"];
    ...
    for (; l < llGetListLength(tex); l++)
    {
        ...
        {
            if (~llSubStringIndex(but, ele))
                but = GetShortName(but);
            else
                but = "";
        }
...
{
    string sPrompt = "\n
<pre>textures - this menu
<pre>settexture [element name] [texturename or UUID] eg <pre>settexture Metal Celtic
...
SetElementTexture(string sElement, string sTex)
{
    ...
    if(~i)
        sTex=llList2String(g_lNotecardTextureKeys,i);
    ...
    if (~iIndex)
        g_lTextures = llListReplaceList(g_lTextures, [sTex], iIndex + 1, iIndex + 1);
    else
        g_lTextures += [s_CurrentElement, sTex];
    //save to settings
    ...
    else if (kID != g_kWearer && iNum != COMMAND_OWNER) return TRUE;
    {
        ...
        else
        {
            ...
            if (llToLower(llList2String(lParams,0))=="settexture")
            {
                ...
                {
                    ok=FALSE;
                    if (llStringLength(sTex) == 36) //ng not valid else if ((key)sTex)
                        ok=TRUE;
                    else
                    {
                    ...
                    if(ok)
                        ...
                        Notify(kID, "The texture " + sTex + " wasn't found in "+CTYPE+" inventory or the textures notecard, please check your command and try again.",FALSE);
                }
            }
        }
    }
...
default
{
    state_entry()
    {
    if (llGetInventoryType( g_sDefTextureCard ) != 7)//if .defaultsettings noteard don't exist then make one
            iwMakeNotecard( g_sDefTextureCard, [texture_notecard] );
                ...
                if(llStringTrim(sData,STRING_TRIM)!="" && llGetSubString(sData,0,1)!="//")
                {
                    ...
                    if(kTextureKey)
                    {
                        ...
                        else if(llGetInventoryType(sTextureName)!=INVENTORY_TEXTURE) // let's not add things that are in inventory.
                        {
                            ...
                            llOwnerSay(sTextureName+" in notecard ignored as already found in inventory. If you remove this texture from inventory, edit the textures notecard (add and delete a blank line) then save the card again to re-read it.");
                     }
                     else
                        llOwnerSay("Texture key for "+sTextureName+" in textures notecard not recognised, dropping.");
                }
                ++g_iNotecardLine;
                g_kNotecardRead=llGetNotecardLine(g_sTextureCard,g_iNotecardLine);
            }
