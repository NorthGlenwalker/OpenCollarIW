...
{
    string sPrompt = "
<pre>options - this menu
<pre>dump_cache = " + DUMPCACHE + "
<pre>fixmenus = " + REFRESH_MENU + "
No Button - no direct button related collar commands\n" + DUMPCACHE + " prints current settings to chat.";
    ...
    if (USER_PREF)
    {
        sPrompt += "\nUncheck " + PREFDESI + " to use designer settings.\n";
        lButtons += PREFDESI;
    }
    else
    {
        sPrompt += "\nCheck " + PREFUSER + " to use personal settings.\n";
        lButtons += PREFUSER;
    }
    
    if (STEALTH)
    {
        sPrompt += STEALTH_ON + " the Collar is hidden.";
        lButtons += STEALTH_ON;
    }
    else
    {
        sPrompt += STEALTH_OFF + " the Collar is shown.";
...
integer UserCommand(integer iAuth, string sStr, key kID)
{
    ...
    {
        llOwnerSay("
<pre>prefix - shows your current prefix (default - first letters of your first and last name)
<pre>prefix aa - sets your prefix, in this case to 'aa'
<pre>channel - shows the current channels the collar is listenning on - default is 0 and 1
<pre>channel 2 - sets your other command channel in this case it will now be 0 and 2
<pre>channel -1 - turns off nearby chat liseners and it will only respond to touch
<pre>safeword - shows the current safeword default is RED.
<pre>safeword HIPPO - changes the default safeword, in this case to 'HIPPO'
<pre>verbose=on - (default) turns on printing to chat if too long to display everything in dialog box
<pre>verbose=off - turns off printing to chat on long list eg RLV menus and nearby sit objects
<pre>settings - displays a list of settings from several scripts in the collar(Garble,Color,Texture,Badwords,(list, penance, animation, sound))
");
return FALSE;
    } 
    else
