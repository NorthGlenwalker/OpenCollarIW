...
integer UserCommand(integer iAuth, string sStr, key kAv)
{
    ...
    if (llToLower(sStr) == "menu titler")
    {
        ...
        {
            sPrompt = "<pre>menu titler - this menu\n<pre>title [text] sets the title to [text]\n<pre>titleon - turns the titler on\n<pre>titleoff - turns the titler off\nCurrent Title: " + g_sText ;
            if(g_iOn == TRUE)
                ON_OFF = ON ;
            else
                ON_OFF = OFF ;
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        else if (iNum == LM_SETTING_RESPONSE)
        {
            ...
            if (sGroup == g_sScript)
            {
                if(sToken == "title")
                    g_sText = sValue;
                if(sToken == "on")
                    g_iOn = (integer)sValue;
                if(sToken == "color")
                    g_vColor = (vector)sValue;
                if(sToken == "height")
                    g_vPrimScale.z = (float)sValue;
                if(sToken == "auth")
                    g_iLastRank = (integer)sValue; // restore lastrank from DB
            }
        ...
        else if (iNum == DIALOG_RESPONSE)
        {
            ...
            {   //response from our main menu
                ...
                {
                    if (sMessage == UP)
                        UserCommand(iAuth, "titleup", kAv);
                    else if (sMessage == DN)
                        UserCommand(iAuth, "titledown", kAv);
                    else if (sMessage == OFF)
                        UserCommand(iAuth, "titleon", kAv);
                    else if (sMessage == ON)
                        UserCommand(iAuth, "titleoff", kAv);
                    UserCommand(iAuth, "menu titler", kAv);
                }
            }
            else if (kID == g_kColourDialogID)
            {  //response form the colours menu
                list lMenuParams = llParseString2List(sStr, ["|"], []);
                key kAv = (key)llList2String(lMenuParams, 0);
                string sMessage = llList2String(lMenuParams, 1);
                integer iPage = (integer)llList2String(lMenuParams, 2);
