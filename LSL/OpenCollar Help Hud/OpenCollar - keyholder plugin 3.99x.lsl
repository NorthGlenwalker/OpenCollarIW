...
//
// OC3.99x by North Glenwalker
// - sanitized script and adjusted for OpenSim
//
...
////////////////////////////////////////////////////////////////////////////////////
// ------------------------------------------------------------------------------ //
//                      OpenCollar - keyholder plugin 3.99x                       //
//                                 version 3.995                                  //
// ------------------------------------------------------------------------------ //
...
string g_szParentmenu = "Apps"; // name of the menu, where the menu plugs in, should be usually Addons. Please do not use the mainmenu anymore ( AddOns or Main is recomended depending on the toy. )
...
BuildLockElementList() // EB :kc was here
{
    ...
    for (n = 2; n <= iLinkCount; n++)
    {
        ...
        if (sPrimName=="locked key" || (llListFindList(lParams, [ "ClosedKey" ]) != -1))
            g_lElementsLockedKey += [n];
        else if (sPrimName=="unlocked key" || (llListFindList(lParams, [ "OpenKey" ]) != -1))
...
{
//ng    list tmp = llParseStringKeepNulls((str = "") + str, [search], []);
    list tmp = llParseString2List(str, [search], []);
    return llDumpList2String(tmp, replace);
}
...
{
    string prompt="<pre>keyholder - this menu
<pre>khreturnkey - returns key
<pre>khtakekey - takes this key
<pre>khforcereturn - forces the key to be returned
<pre>khtimerreturn - forces a timer return of the key
<pre>khlockout - collar locks when key taken
<pre>khsetmainmenu - locksout wearer from main menu
<pre>khunsetmainmenu - unlocks main menu
<pre>khsetglobal - set Global lockout
<pre>khunsetglobal - unset global lockout";
...
//                  [23:45:29]  Kadah Coba: 1h, 3h, 6h, 1d, 2d, 3d?, 1w
    ...
    prompt = 
"<pre>khsettimerreturnkey - this menu
<pre>khunsettimerreturnkey - turns off return key timer
...
DoMenuConfigure(key keyID, integer page, integer iAuth)
{
    ...
    list mybuttons = localbuttons;
    prompt = 
"<pre>khseton - turns on keyholder plugin
<pre>khunseton - turns off keyholder plugin
<pre>khsetlock - locks the collar when the key is taken
<pre>khunsetlock - unlocks the collar
<pre>khsetnoopen - set to not open
<pre>khunsetnoopen - set to open
<pre>khsetpub.key - sets the key to public take
<pre>khunsetpub.key - usets public access
<pre>khsetautooff - turns off autoreturn
...
TakeKey(key avatar, integer auth, integer remote)
{
    ...
    if (kh_auto_return_timer == TRUE && kh_auto_return_time == TRUE)
    {
        ...
        llMessageLinked(LINK_WHAT, COMMAND_OWNER, "timer real=" + (string)hours + ":" + (string)minutes,NULL_KEY);
        llMessageLinked(LINK_WHAT, COMMAND_OWNER, "timer START", NULL_KEY);//NG
    }
    ...
    if (remote == FALSE && g_iGlobalKey == TRUE)
        llWhisper(g_iKeyHolderChannel, llDumpList2String(["key", "take", (string)avatar, (string)auth], ";") );
    updateVisible();
    saveSettings();
}
...
ReturnKey(string reason, integer remote)
{
    ...
    kh_name = "";
    kh_type = 0;
    if (kh_disable_openaccess == TRUE && kh_saved_openaccess == TRUE && oc_openaccess == FALSE)
        llMessageLinked(LINK_WHAT, COMMAND_OWNER, "setopenaccess", avatar); 
    if (kh_lock_collar == TRUE && kh_saved_locked == FALSE && oc_locked == TRUE)
...
updateVisible()
{
    ...
    else if (oc_locked == TRUE)
    {
        if (kh_key == NULL_KEY && kh_on == TRUE)
        {
    ...
    else
    {
        ...
        if (kh_on == FALSE && g_iHideLockWhenOff == TRUE) // just hide the thing entirely in this case.
            SetElementAlpha(0.0, g_lElementsUnlockedLock);
        else
...
loadLocalSettings(string sSettings)
{
    list lValues = llParseStringKeepNulls(sSettings, [ "," ], []);
    if (kh_key != NULL_KEY)
        return; // Already have a keyholder, do not overwrite.
...
integer UserCommand(integer num, string str, key id) // here iNum: auth value, sStr: user command, kID: avatar id
{
    ...
    // Main Menu
    if (str == "menu " + g_szSubmenu || str =="keyholder")
        DoMenu(id, 0, num);
    // Config Menu
    ...
    {
        if (KeyholderCheck(id) == TRUE)
            ReturnKey("", FALSE);
    }
    else if (kh_public_key && str == "khtakekey")
    {
        if (id == g_keyWearer)
            Notify(id, "Taking your own key does not make any sense.", FALSE);
        else if (kh_key != NULL_KEY)
            Notify(id, "The key is not in the lock.", FALSE);
        else
    ...
    else if (llGetSubString(str, 0, 4) == "khset" && (llGetSubString(str, 5, 6) == "0," || (integer)llGetSubString(str, 5, 5) > 0) )
    {
        if (OwnerCheck(id, num) == FALSE)
            return TRUE;
...
default
{
    ...
    {
        if (llGetOwner()!=g_keyWearer)// Reset if wearer changed
            llResetScript();
        if ( kh_lockout == TRUE || kh_key != NULL_KEY )
            llMessageLinked(LINK_WHAT, WEARERLOCKOUT, "on", "");
        updateVisible();
    }
    ...
    {
        if (num == MENUNAME_REQUEST) // our parent menu requested to receive buttons, so send ours
        {
            if (str == g_szParentmenu)
            {
                llMessageLinked(LINK_WHAT, MENUNAME_RESPONSE, g_szParentmenu + "|" + g_szSubmenu, "");
                llMessageLinked(LINK_SET, MENUNAME_REQUEST, g_szSubmenu, "");//NG
            }
            if (str == g_szTimerMenu)
                setTimerMenu();
        }
        else if (num == MENUNAME_RESPONSE) // a button is send to be added to a menu
        {
            list parts = llParseString2List(str, ["|"], []);
            if (llList2String(parts, 0) == g_szSubmenu)
            {//someone wants to stick something in our menu
                string button = llList2String(parts, 1);
                if (llListFindList(externalbuttons, [button]) == -1) // if the button isnt in our menu yet, than we add it
                    externalbuttons = llListSort(externalbuttons + [button], 1, TRUE);
            }
        }
        else if (num == MENUNAME_REMOVE) // a button is send to be removed from a menu
        {
            list parts = llParseString2List(str, ["|"], []);
            if (llList2String(parts, 0) == g_szSubmenu)
            {//someone wants to stick something in our menu
        ...
        { // Do nothing, they are not allowed.
            if (str == "menu" && ( kh_key != NULL_KEY || kh_lockout ) )
            {
                if ( kh_key == NULL_KEY )
                    ...
                    Notify(g_keyWearer, "You are locked out of the " + g_sToyName + " until your key is returned.", TRUE);
            }
        }
        else if (num == COMMAND_EVERYONE)
        {
            if (kh_public_key == TRUE && str == "khtakekey")
            {
                if (kh_key != NULL_KEY)
                    Notify(id, "The key is not in the lock.", FALSE);
                else
            ...
            else if (kh_key == NULL_KEY && str == "menu" && oc_openaccess == FALSE && kh_public_key == TRUE)
                DoMenuSpecial(id, 0, TRUE, num);
        }
        else if (UserCommand(num, str, id))
            return;  // from COMMAND_OWNER to COMMAND_WEARER
            ...
            // you have to parse the answer from the dialog system and use the parsed variable av
        {
            ...
            if (id == g_keyMenuID || id == g_keyConfigMenuID || id == g_keyConfigAutoReturnMenuID)
            {
                ...
                if (message == UPMENU)
                {
                    ...
                    UserCommand(iAuth, "khreturnkey", av);
        ...
        else if (num == LM_SETTING_RESPONSE || num == LM_SETTING_SAVE)
        {
            ...
            if ( CompareDBPrefix(token, "Global_locked") == TRUE )//NG changed from "locked"
            {
                oc_locked = (integer)value;
                updateVisible();
            }
            else if ( CompareDBPrefix(token, "openaccess") == TRUE )
                oc_openaccess = (integer)value;
            else if ( CompareDBPrefix(token, TOK_STORE) == TRUE )
                loadDBSettings(value);
        }
        else if (num == LM_SETTING_DELETE)// Saddly it's deleted to indicate FALSE rather than set to 0...
        {
            if ( CompareDBPrefix(str, "Global_locked") == TRUE)
            {
                oc_locked = FALSE;
                updateVisible();
            }
            else if ( CompareDBPrefix(str, "openaccess") == TRUE)
                oc_openaccess = FALSE;
        }
        ...
        else if (num == TIMER_EVENT)
        {
            if (str == "end")
            {
                if (kh_auto_return_timer == TRUE)
                    ReturnKey("The automatic timer has expired.", FALSE);
                else if (kh_return_on_timer == TRUE)
    ...
    listen(integer channel, string name, key id, string message)
    {
        ...
        {
            if (g_iGlobalKey == FALSE)
                return; // don't care.
            if (llGetOwner() != llGetOwnerKey(id))
                return; // Not for us.
            list lArgs = llParseString2List(message, [";"], []);
            if (llList2String(lArgs, 0) != "key") 
