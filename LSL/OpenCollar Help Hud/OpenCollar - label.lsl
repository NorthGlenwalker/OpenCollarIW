...
SetOffsets(key font)
{
    ...
    if (desc == "Label")
    {
        ...
        {
            if (font == NULL_KEY)
                font = "278550c4-d65d-4430-bac8-d9668ba092c6"; // LCD default for box
        ...
        {
            if (font == NULL_KEY)
              font = "d71ee1d2-1fba-4c83-b4aa-82ac3d3ee5a3"; // Serif default for cyl
...
MainMenu(key kID, integer iAuth)
{
    ...
    if (g_iScroll)
        ...
        lButtons += ["☐ Scroll"];    
    string sPrompt = "<pre>label - this menu
<pre>labeltext [text] - sets the label to [text]
<pre>labelcolor <color code> - -sets the lavel text to this color
<pre>labelshow - hides the label
<pre>labelshow 1 - shows the label
<pre>labelscroll - stops the text from scrolling
<pre>labelscroll 1 - starts the text scrolling
...
integer UserCommand(integer iAuth, string sStr, key kAv)
{
    ...
    if (iAuth == COMMAND_OWNER || !g_iAppLock)
    {
        ...
        {
            if (llToLower(llList2String(lParams, 1)) == "0")
                g_iAppLock = FALSE;
            else
                g_iAppLock = TRUE;
        }        
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        else if (iNum == LM_SETTING_RESPONSE)
        {
            ...
            if (llGetSubString(sToken, 0, i) == g_sScript)
            {
                ...
                if (sToken == "text")
                    ...
                    SetOffsets((key)sValue);
                else if (sToken == "color")
                    g_vColor = (vector)sValue;
                else if (sToken == "show")
                    g_iShow = (integer)sValue;
                else if (sToken == "scroll")
                    g_iScroll = (integer)sValue;                
            }
        ...
        else if (iNum == DIALOG_RESPONSE)
        {
            ...
            else if (kID == g_kTBoxID) // TextBox response, extract values
            {
                ...
                integer iAuth = (integer)llList2String(lMenuParams, 3);
                if (sMessage != "")
                    UserCommand(iAuth, "labeltext " + sMessage, kAv);
