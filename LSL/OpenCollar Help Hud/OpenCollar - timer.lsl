...
DoMenu(key keyID, integer iAuth)
{
    ...
    { }
        ...
        return;
    string sPrompt = "<pre>timer - this menu
<pre>timer START - starts the timer (if set)
<pre>timer STOP - ends the timer
<pre>timer RESET - resets the timer to 00;00
...
DoOnMenu(key keyID, integer iAuth)
{
    ...
    string sPrompt = "<pre>timer Online - this menu\n Online Time Settings\n";
    sPrompt += "\n Online Timer: "+Int2Time(g_iOnSetTime);
    if (g_iOnRunning)
        sPrompt += "\n Online Timer: "+Int2Time(g_iOnTimeUpAt-g_iOnTime)+" left";
    else
...
DoRealMenu(key keyID, integer iAuth)
{
    if (keyID == NULL_KEY)
        return;
    string sPrompt = "<pre>timer RL - this menu\n RL Time Settings\n";
    sPrompt += "\n RL timer: " + Int2Time(g_iRealSetTime);
    if (g_iRealRunning)
        sPrompt += "\n RL Timer: "+Int2Time(g_iRealTimeUpAt-g_iCurrentTime)+" left";
    else
...
default
{
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        else if (iNum == LM_SETTING_SAVE)
        {
            ...
            if (token == "Global_locked" && (integer)value == 1)
                g_iCollarLocked = 1;
        ...
        else if (iNum == MENUNAME_RESPONSE) // a button is sned ot be added to a plugin
        {
            ...
            {//someone wants to stick something in our menu
                ...
                if (llListFindList(lButtons, [sButton]) == -1) // if the button isnt in our benu yet, than we add it
            ...
            {//this is one of our menus
                ...
                else if (kID == g_kOnMenuID)
                {
                    if (sMsg == UPMENU)
                       ...
                       UserCommand(iAuth, "timer on"+sMsg, kAv);
                }
                else if (kID == g_kRealMenuID)
                {
                    if (sMsg == UPMENU)
                       ...
                       UserCommand(iAuth, "timer real"+sMsg, kAv);
                }                  
            }          
        }
    }
