...
LeashMenu(key kIn, integer iAuth)
{
    ...
    if (g_iStay)
        ...
        if (g_iRot)
            ...
            lButtons += [BUTTON_ROT];
    string sPrompt = "\n<pre>leashmenu - this menu some options are wearer or non wearer only press each button for the chat commnads
...
integer UserCommand(integer iAuth, string sMessage, key kMessageID)
{
    if (iAuth >= COMMAND_OWNER && iAuth <= COMMAND_WEARER)
    {
        ...
        if (sMesL == "grab" || sMesL == "leash" || (sMesL == "toggleleash" ))
            llOwnerSay("<pre>leash or <pre>grab or <pre>toggleleash - grab this persons leash");
        else if(sComm == "follow")
            llOwnerSay("<pre>follow - leash them without leash showing");
        else if (sMesL == "runaway" && iAuth == COMMAND_OWNER)
            Unleash(kMessageID);
        else if (sMesL == "unleash" || sMesL == "unfollow" || (sMesL == "toggleleash" && NULL_KEY != g_kLeashedTo))
            llOwnerSay("<pre>unleash or <pre>unfollow or <pre>toggleleash - removes the leashing");
        else if (sMesL == "giveholder")
            llOwnerSay("<pre>giveholder - gives you a leash holder");
        else if (sMesL == "givepost")
            llOwnerSay("<pre>give post - gives you a grabby post");
        else if (sMesL == "rezpost")
            llOwnerSay("<pre>rezpost - rezzes a grabby post next to the wearer");
        else if (sMesL == "yank" ) //Person holding the leash can yank.
            llOwnerSay("<pre>yank - person you are leashed to pulls you closer");
        else if (sMesL == "beckon") //Owner can beckon
            llOwnerSay("<pre>beckon - pulls you closer to the collar owner");
        else if (sMesL == "stay")
            llOwnerSay("<pre>stay - forces the wearer to stay where thay are");
        else if ((sMesL == "unstay" || sMesL == "move") && g_iStay)
        {
            llOwnerSay("<preunstay or <pre>move - releases wearer from stay");
        }
        else if(sMesL == "don't rotate" && g_iRot)
        {
            llOwnerSay("<pre>don't rotate - stops the wearer turning around when pulled backwards");
        }
        else if(sMesL == "rotate" && !g_iRot)
        {
            llOwnerSay("<pre>rotate - allows the wearer to turn toward where they leash is pulling them");
        }
        ...
        {
            llOwnerSay("<pre>leashto - brings up the menu to select a person to leash to");
        }
        else if(sComm == "followtarget")
        {
            llOwnerSay("<pre>followtarget - brings up the menu to select person to follow");
        }
        else if (sComm == "length")
        {
            ...
            if(fNewLength > 0.0)
            {
                ...
                if (kMessageID == g_kLeashedTo || CheckCommandAuth(kMessageID, iAuth)) 
                {
                    ...
                    //tell wearer  
                    Notify(kMessageID, "<pre>length - displays the currect leash length\nLeash length set to " + sVal, TRUE);        
                    llMessageLinked(LINK_SET, LM_SETTING_SAVE, g_sScript + TOK_LENGTH + "=" + sVal, "");
                }
            }
            else Notify(kMessageID, "<pre>length - displays the currect leash length\nThe current leash length is " + Float2String(g_fLength) + "m.", TRUE);
        }
        else if (sComm == "leashtopost")
        {
            llOwnerSay("<pre>leashtopost - brings up menu of nearby objects to leash wearer to");
            if (!CheckCommandAuth(kMessageID, iAuth))
                return TRUE;
