...
default
{
    ...
    listen(integer channel, string sName, key kID, string sMessage)
    {
        ...
        {//process emotes, replace with sub name
            if(llGetSubString(sMessage, 0, 3) == "/me ")
                sMessage=g_sSubName + llGetSubString(sMessage, 3, -1);
            else
                sMessage=g_sSubName + ": " + sMessage;
            ...
            while (iMessageLength > g_iListenCap)
            {  //if message longer than allowed buffer length, trim it and replace the last few bytes with warning string
    ...
    timer (){
        ...
        {  //timeout during init.  We waited long enough for a random send of our data.  Ask for any that's missing
            if (! g_iGotSettingOwners)
                llMessageLinked(LINK_THIS,LM_SETTING_REQUEST,"auth_owner",NULL_KEY);
            if (! g_iGotSettingTrace)
                llMessageLinked(LINK_THIS,LM_SETTING_REQUEST,"subspy_trace",NULL_KEY);
            if (! g_iGotSettingRadar)
                llMessageLinked(LINK_THIS,LM_SETTING_REQUEST,"subspy_radar",NULL_KEY);
            if (! g_iGotSettingListen)
                llMessageLinked(LINK_THIS,LM_SETTING_REQUEST,"subspy_listen",NULL_KEY);
            if (! g_iGotSettingMeters)
                llMessageLinked(LINK_THIS,LM_SETTING_REQUEST,"subspy_meters",NULL_KEY);
            if (! g_iGotSettingMinutes)
                llMessageLinked(LINK_THIS,LM_SETTING_REQUEST,"subspy_minutes",NULL_KEY);
            g_sState="postInit";
            llSetTimerEvent(5.0);
        }
    ...
    sensor(integer iNum)
    {
        ...
        {//put nearby avs in list
            list lAvBuffer;
            while
                (iNum) lAvBuffer += llDetectedName(--iNum);
