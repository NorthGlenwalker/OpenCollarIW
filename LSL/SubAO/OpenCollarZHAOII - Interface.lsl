...
{
    list utility = [];
    list buttons = ["AO Menu","Couples","Collar Menu","Pose","Sits","Ground Sits","Reset","HUD Style","Help"];
...
default 
{
    ...
    link_message(integer sender, integer num, string str, key id)
    {
        if (num >= COMMAND_OWNER && num <= COMMAND_WEARER)
        {
            ...
            else if (str == "ZHAO_AOON") //make sure button is bright, on is TRUE
                TurnOn();
            ...
            if (menuindex != -1)
            {
                ...
                else if (menutype == QUICKMENU)
                {
                    if (_message == "AO Menu")
                        ...
                        llMessageLinked(LINK_THIS, COMMAND_AUTH, "couples", _id); //moved link to internal couples
    ...
    { //ignore touches when attached not at the hud and touches by others but the wearer
        ...
        if( (llGetAttached() ) && ( llDetectedKey(0) == Owner) ) 
        {
            ...
            string message = "";
            if (button == "Menu")  // -- message = "ZHAO_MENU";
                message = "OCAO_MENU";
