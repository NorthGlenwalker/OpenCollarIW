...
string parentmenu = "Main1"; // changed for Owner HUD
...
integer SUBMENU = 42;//renumbered for SubAO intergration
integer DIALOG = -9000;
        ...
        if (id == dataid)
        {
            if (data == EOF)
            {
                if(noteCard2Read == CARD1)
                {
                    ...
                    else // no Mycoupleanims notecard so...
                        state ready;
                }
                else
                    state ready;
...
//                        llMessageLinked(LINK_THIS, SUBMENU, parentmenu, id);
                        llMessageLinked(LINK_THIS, COMMAND_OWNER, "OCAO_MENU", id);
                ...
                else if (menutype == PARTNERMENU)
                {
                    ...
                    {
                        timeout = 0.0;
                        llOwnerSay("Couple Anmiations play now for ever. Use the menu to stop them again.");
                        if (id != wearer)
                            llInstantMessage(id, "Couple Anmiations play now for ever. Use the menu to stop them again.");
                        CoupleAnimMenu(id);
                    }           
                    else
                    {
